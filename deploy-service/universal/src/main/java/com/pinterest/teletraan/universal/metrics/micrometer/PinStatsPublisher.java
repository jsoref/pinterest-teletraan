...
public class PinStatsPublisher {
  ...
  protected Stream<String> writeFunctionTimer(FunctionTimer timer) {
    ...
    return Stream.of(
        ...
        writeMetricWithSuffix(timer.getId(), "sum", wallTime, timer.totalTime(baseTimeUnit)));
  }
  ...
  private List<String> writeHistogram(
    ...
    metrics.add(
        writeMetricWithSuffix(
            meter.getId().withTag(new ImmutableTag("le", "+Inf")), "bucket", wallTime, count));
  ...
  protected String writeMetricWithSuffix(Meter.Id id, String suffix, long wallTime, double value) {
  ...
  public void publish(List<Meter> metrics) {
    Flux<String> metricsFlux = Flux.fromIterable(metrics)
        .flatMap(
            m -> Flux.fromStream(
                m.match(
                    this::writeGauge,
    ...
    Connection connection = TcpClient.create()
        ...
        .port(config.port())
        .handle(
            (inbound, outbound) -> outbound.sendString(
                metricsFlux
                    .onErrorResume(
                        e -> {
                          LOG.warn("failed to send to metrics agent", e);
                          return Mono.empty();
                        })
                    .doOnComplete(() -> LOG.debug("sent {} metrics", metrics.size()))))
        .connectNow();
