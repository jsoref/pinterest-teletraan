...
class TestBuilds(unittest.TestCase):
    def test_build_apis(self):
        ...
        builds = builds_helper.get_builds(commons.REQUEST, name="build1", pageIndex=1, pageSize=1)
        build2 = builds[0]
        self.assertTrue(len(builds) == 1)
        self.assertTrue(build1 == build2)

        builds = builds_helper.get_builds(commons.REQUEST, commit="commit-1", pageIndex=1,
                                          pageSize=2)
        self.assertTrue(len(builds) == 2)

        builds = builds_helper.get_builds(commons.REQUEST, name="build2", pageIndex=1, pageSize=2)
        self.assertTrue(len(builds) == 2)

        build_names = builds_helper.get_build_names(commons.REQUEST, pageIndex=1, pageSize=2)
